...
class DataLoader(Sequence):
    def __init__(self, datapath, batch_size, height_hr, width_hr, scale, crops_per_image):
        ...
        if self.datapath.startswith('gs://'):
            self.img_paths = []
            for dirpath, _, filenames in tf.gfile.Walk(self.datapath):
...
def plot_test_images(model, loader, datapath_test, test_output, epoch, name='SRGAN'):
    ...
    try:
        ...
        if datapath_test.startswith('gs://'):
            test_images = []
            for dirpath, _, filenames in tf.gfile.Walk(datapath_test):
                for filename in [f for f in filenames if any(filetype in f.lower() for filetype in ['jpeg', 'png', 'jpg'])]:
        ...
        # Plot the images. Note: rescaling and using squeeze since we are getting batches of size 1                    
            ...
            # Save directory            
            file_name = "{}-Epoch{}.png".format(filename, epoch)
            # save the plot
            if test_output.startswith('gs://'):
                fig.savefig(file_name)
                copy_file_to_gcs(job_dir=test_output, file_path=file_name)
            else:
                file_name = os.path.join(test_output, file_name)
                fig.savefig(file_name)
...
class ContinuousExport(tf.keras.callbacks.Callback):
    ...
    def __init__(self, eval_frequency, log_dir):
    ...
    def on_epoch_end(self, epoch, logs={}):
        if epoch >= 0 and epoch % self.eval_frequency == 0:
            ...
            if len(checkpoints) > 0:
                print('>> Copying the model {} to {}'.format(checkpoints[-1], self.log_dir))
                copy_file_to_gcs(self.log_dir, checkpoints[-1])
        else:
