...
from trainer.libs.util import DataLoader, plot_test_images, ContinuousExport, load_weights_from_gcs, copy_file_to_gcs
...
class SRGAN():
    ...
    def __init__(self, 
        ...
        height_lr=48, width_lr=48, channels=3,
        upscaling_factor=4,
        ...
        if self.generator_weights is not None:
            if self.generator_weights.startswith('gs://'):
                # write a local temp file to read weights from
                print('Setting the generator weights to {}'.format(self.generator_weights))
                self.generator_weights = load_weights_from_gcs(self.generator_weights)
            # load the file
            self.generator.load_weights(self.generator_weights)
        ...
        if training_mode:
            ...
            self.discriminator, self.frozen_discriminator = self.build_discriminators()
            if self.discriminator_weights is not None:
                print('Setting the discriminator weights to {}'.format(self.discriminator_weights))
                self.discriminator.load_weights(self.discriminator_weights)
                self.frozen_discriminator.load_weights(self.discriminator_weights)
                print('Done with weights')
    ...
    def save_weights(self, weight_path, dataname):
        """Save the generator and discriminator networks"""

        gen_name = "{}_generator_{}X.h5".format(dataname, self.upscaling_factor)
        dis_name = "{}_discriminator_{}X.h5".format(dataname, self.upscaling_factor)

        if weight_path.startswith('gs://'):
            self.generator.save_weights(gen_name)
            self.discriminator.save_weights(dis_name)
            copy_file_to_gcs(job_dir=weight_path, file_path=gen_name)
            copy_file_to_gcs(job_dir=weight_path, file_path=dis_name)
        else:
            gen_name = os.path.join(weight_path, gen_name)
            dis_name = os.path.join(weight_path, dis_name)
            self.generator.save_weights(gen_name)
            self.discriminator.save_weights(dis_name)
    ...
    def build_discriminators(self, filters=64):
        ...
        frozen_discriminator = Network(inputs=img, outputs=x, name='frozen_discriminator')
    ...
    def train_generator(self,
        ...
        # Checkpoints
        checkpoint_path = 'checkpoint.{epoch:04d}-{val_loss:.9f}-' + dataname + '_{}.h5'.format(self.upscaling_factor)

        if not job_dir.startswith("gs://"):  # then local
            checkpoint_path = os.path.join(log_weight_path, checkpoint_path)
        ...
        # cp_name = dataname + '_{}X.h5'.format(self.upscaling_factor)
        # if job_dir.startswith('gs://'):
        #     cp_path = cp_name
        # else:
        #     cp_path = os.path.join(log_weight_path, cp_name)
        ...
        # Callback: export model to GCS
        if job_dir.startswith('gs://'):
            model_export = ContinuousExport(eval_frequency=1, log_dir=log_weight_path)
            callbacks.append(model_export)
    ...
    def train_srgan(self, 
        ...
        for step in range(0, steps_per_epoch * int(epochs)):
            ...
            if datapath_test and epoch_change and current_epoch % log_test_frequency == 0:
                plot_test_images(self, loader, datapath_test, log_test_path, current_epoch)
