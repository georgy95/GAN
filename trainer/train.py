...
from tensorflow import ConfigProto, Session, GPUOptions
...
from trainer.libs.util import plot_test_images, DataLoader
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        type=str, default='./trainer/data/weights/',
        help='Where to output weights during training'
    )
        
    parser.add_argument(
        '-log_path', '--log_path',
        type=str, default='./trainer/data/logs/',
    ...
    parser.add_argument(
        ...
        help='Type of the job =remote for CloudML'
    )

    parser.add_argument(
        '-weights', '--weights',
        type=str, default=None,
        help='Custom directory to load gen/dis weights'
    )
...
def gan_train(args, gan, common, first_step=0):
...
if __name__ == '__main__':
    ...
    # # config.gpur_options.allow_growth = True
    ...
    srresnet_path = os.path.join(common['log_weight_path'], 'SRResNet_'+args.dataname+'_{}X.h5'.format(args.scale))
    srrgan_G_path = os.path.join(common['log_weight_path'], 'SRGAN_'+args.dataname+'_generator_'+str(args.scale)+'X.h5')
    srrgan_D_path = os.path.join(common['log_weight_path'], 'SRGAN_'+args.dataname+'_discriminator_'+str(args.scale)+'X.h5')
    ...
    if args.stage in ['all', 'mse']:
        if args.scaleFrom:
            ...
            weights = 'gs://gr-20190331/srgan/cloudml/jobs/gan_training_remote_20190330_162234/weights/SRResNet_imagenet_4X.h5'
            gan = SRGAN(upscaling_factor=args.scale, generator_weights=weights)    
    ...
    if args.stage in ['all', 'gan']:
        if args.stage=='gan':
            # not complete training -> set generator initial weights
            srresnet_path = 'gs://gr-20190331/srgan/cloudml/jobs/gan_training_remote_20190330_162234/weights/SRResNet_imagenet_4X.h5'
        gan = SRGAN(upscaling_factor=args.scale, generator_weights=srresnet_path)
        gan_train(args, gan, common, first_step=0)
